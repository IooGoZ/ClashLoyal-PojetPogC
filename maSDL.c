...
void prepareAllSpriteDuJeu(t_plateauJeu jeu, SDL_Surface **TabSprite, SDL_Surface* destination ) {
    for (int j=0;j<jeu.maxY;j++){
        for (int i=0;i<jeu.maxX;i++){
            if (jeu.plateau[i][j] != NULL){
                prepare_sprite(i*40,j*40,TabSprite[jeu.plateau[i][j]->nom],destination);  //remarque les sprites font 40*40 pixels
